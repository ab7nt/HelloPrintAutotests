...
export class ProductCalculationRegisterPage {
   constructor(page) {
      this.page = page
      ...
      // this.columnWithOrderStatuses = page.locator('div[tabulator-field="status"].tabulator-cell')

      // Локаторы для контрагента
      this.partnerFilterField = this.mainBlockFields.nth(0)
      ...
      this.columnWithOrderPartner = page.locator('div[tabulator-field="partner_id"].tabulator-cell')
      // Локаторы менеджера расчёта
      this.managerFilterField = this.mainBlockFields.nth(1)
      this.managerFilterInput = page.locator('div:has(select[name="user_id[]"]) > span input')
      this.optionListAfterFillManager = page.locator('ul[id*="select2-user_id"] li')
      this.columnWithOrderManagers = page.locator('div[tabulator-field="manager_id"].tabulator-cell')
      // this.resetAllFiltersButton = page.locator('div.cancel-search-button')

      // Локаторы компании изделия
      ...
      // Локаторы наименования изделия
      // this.nomenclatureNameFilterField = this.mainBlockFields.nth(3)
      // this.nomenclatureNameFilterInput = page.locator('div:has(select[name="nomenclature_names[]"]) > span input')
      // this.optionListAfterFillNomenclatureName = page.locator('ul[id*="select2-nomenclature_names"] li')
      // this.columnWithOrderNomenclatureName = page.locator('div[tabulator-field="name"].tabulator-cell')
      ...
      // this.columnWithOrderErexpress = page.locator('div[tabulator-field="is_express"].tabulator-cell')
      ...
      // this.columnWithOrderPartnerType = page.locator('')

      // // Локаторы для ИНН контрагента
      // this.partnerCompanyInnFilterField = this.partnerBlockFields.nth(2)
      ...
      // // Локаторы для "Бракованные изделия"
      // this.defectFilterField = this.stageBlockFields.nth(3)
      // this.defectFilterSelect = page.locator('div#filter-block-other select[name="is_defect"]')
      // this.columnWithDefect = page.locator('div[tabulator-field="status"].tabulator-cell')
   }
   ...
   // filteringByStatusProduct = async () => {
   ...
   //    await this.page.waitForLoadState('load')
   //    await expect(this.selectedTags).toContainText(`Статус: ${filtersInfo.productStatus}`)
   ...
   // filteringByNomenclatureName = async () => {
   //    await this.nomenclatureNameFilterField.waitFor()
   //    await this.nomenclatureNameFilterField.click()
   //    await this.nomenclatureNameFilterInput.fill(filtersInfo.nomenclatureName)
   //    await expect(this.nomenclatureNameFilterInput).toHaveValue(filtersInfo.nomenclatureName)
   //    await this.optionListAfterFillNomenclatureName.waitFor()
   //    await this.optionListAfterFillNomenclatureName.click()
   //    await this.submitButton.waitFor()
   //    await this.submitButton.click()
   //    await this.page.waitForLoadState('load')
   //    await expect(this.selectedTags).toContainText(`Наименование: ${filtersInfo.nomenclatureName}`)
   //    await this.rowsRegistryTable.first().waitFor()
   //    await helpers.checkingTextForAnArrayOfElements(filtersInfo.nomenclatureName, this.columnWithOrderNomenclatureName)
   // }
   ...
   //    await this.page.waitForLoadState('load')
   //    await expect(this.selectedTags)
   //       .toContainText(`Выдача: c ${filtersInfo.orderCreateDateBegin} по ${filtersInfo.orderCreateDateEnd}`)
   //    await this.rowsRegistryTable.first().waitFor()
   ...
   //    // Выбор статуса "В работе", чтобы избежать изделий в статусах "Готово" и "Отменено"
   //    await this.statusFilterField.waitFor()
   //    await this.statusFilterField.click()
   //    await expect(this.optionList.first()).toBeVisible()
   //    await this.optionList.first().waitFor()
   //    await this.optionList.filter({ hasText: filtersInfo.productStatus }).click()
   //    // Непосредсвенно проверка участка
   ...
   //    await this.rowsRegistryTable.first().waitFor()
   //    await helpers.checkingTextForAnArrayOfElements(filtersInfo.express.split(" ")[0], this.columnWithOrderErexpress)
   // }
   ...
   // filteringByDefectProduct = async () => {
   //    await this.buttonBlockOther.waitFor()
   //    await this.buttonBlockOther.click()
   //    await this.defectFilterField.waitFor()
   //    await this.defectFilterField.click()
   //    await this.optionList.first().waitFor()
   //    await this.optionList.filter({ hasText: filtersInfo.defect }).click()
   //    expect(await this.defectFilterSelect.locator('option:checked').innerText()).toEqual(filtersInfo.express)
   //    await this.submitButton.waitFor()
   //    await this.submitButton.click()
   //    await this.page.waitForLoadState('load')
   //    await expect(this.selectedTags).toContainText(`Брак>: ${filtersInfo.defect}`)
   //    await this.rowsRegistryTable.first().waitFor()
   //    await helpers.checkingTextForAnArrayOfElements('Брак', this.columnWithDefect)
   // }
}